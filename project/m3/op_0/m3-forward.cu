...
static bool g_use_const_mask = false;
                                  ...
                                  int use_const_mask)
{
    ...
    for (size_t tile_k = 0; tile_k < k_extent; tile_k += TILE_K) {
        if (row < (size_t)Map_out && tile_k + tx < k_extent) {
            const size_t mask_idx = row * k_extent + (tile_k + tx);
            tile_A[ty][tx] = use_const_mask ? const_mask[mask_idx] : mask[mask_idx];
        } else {
            tile_A[ty][tx] = 0.0f;
        }

        if (col < (size_t)Batch * Height_out * Width_out && tile_k + ty < k_extent) {
            size_t tmp = col;
                                                     ...
                                                     const int Width, const int K)
{
    ...
    const int mask_elems = Map_out * Channel * K * K;
    ...
    g_use_const_mask = (mask_elems <= MAX_CONST_MASK_FLOATS);
    if (g_use_const_mask) {
        cudaMemcpyToSymbol(const_mask, host_mask, mask_bytes);
    }
}
